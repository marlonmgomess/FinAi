  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 pb-safe z-20">
      <div className="max-w-2xl mx-auto flex justify-around items-center h-16">
        ...
        >
          <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-colors ${window.location.hash === '#/chat' ? 'bg-emerald-600 text-white' : 'bg-white text-emerald-600 border-2 border-emerald-600'}`}>
            <MessageCircle size={28} />
          </div>
          <span className="text-xs font-bold mt-1 text-slate-600">FinAI</span>
        </NavLink>
