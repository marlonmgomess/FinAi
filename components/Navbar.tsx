...
import { LayoutDashboard, MessageCircle, History, Landmark, Settings } from 'lucide-react';
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 pb-safe z-40">
      <div className="max-w-2xl mx-auto flex justify-around items-center h-16">
        ...
        >
          <Landmark size={18} />
          <span className="text-[9px] font-black uppercase tracking-tighter">Cofre</span>
        </NavLink>
        ...
        >
          <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-xl transition-colors ${window.location.hash.includes('chat') ? 'bg-emerald-600 text-white' : 'bg-white text-emerald-600 border-2 border-emerald-600'}`}>
            <MessageCircle size={24} />
          </div>
          <span className="text-[9px] font-black mt-1 text-slate-600 uppercase">FinAI</span>
        </NavLink>
